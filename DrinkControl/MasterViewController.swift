...
class MasterViewController: UITableViewController {
    ...
    let sectionTitleArray:[String] = ["飲んだお酒の量を入力",
                                      "計算結果（1ドリンク＝アルコール10グラム)","評価とコメント"]
    var drinkDaily = DrinkDailyRecord(dDate: Date(), drinks:[eDname:Int](), totalUnits: 0)
    
//    セグエでの値の受け渡し
    var drinkItem = eDname.wine, damount: 0)
...
//        cancelButton.isEnabled = false
    ...
    private func loadDataForTable(drink:DrinkDailyRecord)  -> [cellArray] {
       ...
       //Section 2 for tableview
        let value = drinkDaily!.evaluation == "" ? "無し": drinkDaily!.evaluation
        let cellText2:[String] = [value]
        
        let item3:cellTupple = (title:cellText2[0], drinkDaily!.comment,style:.disclosureIndicator)
         
         let array_3 = [item3]
        
         return [array_0,array_1,array_3]
         }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "showComment":
                ...
                controller.evaluation = drinkDaily!.evaluation
                controller.comment = drinkDaily!.comment
        default: break
        }
    }
    ...
    @IBAction func unwindToMasterView(sender: UIStoryboardSegue) {
        ...
        if let sourceViewController = sender.source as? DataEntryTableViewController,
            ...
            if let sourceViewController = sender.source as? CommentViewController {
                let newEvaluation = sourceViewController.evaluation
                let newComment = sourceViewController.comment
                drinkDaily!.evaluation = newEvaluation
                drinkDaily!.comment = newComment
